<template>
  <div v-if="device.name">
    <h5 class="center upper-bold accent">{{device.manufacturer}} {{device.name}} ({{device.codename}})</h5>
...
<!--         <div class="builds cardColor z-depth-1" style="border-radius: 25px !important"> -->
          ...
          <div v-if="device.active">
            ...
            <h6 class="link" v-on:click="openLink(device.maintainer_link)">{{device.maintainer_name}}</h6>
            <Flag width="20px" :country="device.maintainer_country" />
            </div>

           <div v-if="device.comaintainer_name" class="deviceprop">
            <i class="material-icons">person_outline</i>
            <h6 class="link" v-on:click="openLink(device.comaintainer_link)">{{device.comaintainer_name}}</h6>
          ...
          <div v-if="device.xda_thread && device.active" class="card-action xda-buttons buildbuttons">
            <a v-bind:href="device.xda_thread" target="_blank" class="btn">XDA Thread</a>
	        </div>
          <div
