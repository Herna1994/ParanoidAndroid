        ...
        <template v-for="romtype in deviceBuilds">
          <template v-for="build in romtype">
            <li
              ...
              class="buildcoll zips"
            >
              <div class="collapsible-header white-text cardColor">
                <i class="material-icons">system_update</i>
                <span class="upper-bold accent" style="width: 100%">{{ build.version }} {{ build.romtype }} {{ build.number }}</span>
  ...
  methods: {
    setBuild(obj) {
      ...
      instances.options.onOpenEnd = () =>
        this.$router.push({ name: "filename", params: { filename: name.toLowerCase() } });
